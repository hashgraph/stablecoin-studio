...
test('[Fireblocks] signTransaction calls the correct strategy with the correct request', async () => {
  ...
  const FireblocksStrategyConfig = new FireblocksConfig(
      FIREBLOCKS_API_KEY,FIREBLOCKS_API_SECRET_KEY,FIREBLOCKS_BASE_URL
  );
  const fireblocksSignatureRequest = new FireblocksSignatureRequest( vaultAccountId, new Uint8Array([1, 2, 3])
  );
  const signatureService = new CustodialWalletService(FireblocksStrategyConfig);
  const signature = await signatureService.signTransaction(fireblocksSignatureRequest);
