...
describe('🧪 [BUILDER] HTSTransactionBuilder', () => {
    ...
    it('Test cashin', async () => {
        ...
        const accountInitialBalance: number = +(await th.balanceOf(stableCoinCapabilities, accountEvmAddress)).response;        
        ...
        const accountFinalBalance: number = +(await th.balanceOf(stableCoinCapabilities, accountEvmAddress)).response;
    ...
    it('Test cashIn contract function', async () => {
        const accountEvmAddress: string = HAccountId.fromString(clientAccountId).toSolidityAddress();
        const accountInitialBalance: number = +(await th.balanceOf(stableCoinCapabilities2, accountEvmAddress)).response;
        ...
        const accountFinalBalance: number = +(await th.balanceOf(stableCoinCapabilities2, accountEvmAddress)).response;
        expect(accountFinalBalance).toEqual(accountInitialBalance + 1);
    }, 20000);

    it('Test burn contract function', async () => {
		const accountEvmAddress: string = HAccountId.fromString(proxyContractId2).toSolidityAddress();
        const accountInitialBalance: number = +(await th.balanceOf(stableCoinCapabilities2, accountEvmAddress)).response;
	...
	it('Test grant role contract function', async () => {
		const accountEvmAddress: string = HAccountId.fromString(accountId).toSolidityAddress();
        tr = await th.grantRole(stableCoinCapabilities2, accountEvmAddress, StableCoinRole.BURN_ROLE);
		tr = await th.hasRole(stableCoinCapabilities2, accountEvmAddress, StableCoinRole.BURN_ROLE);
		expect(tr.response).toEqual(true);
    });

	it('Test revoke role contract function', async () => {
		const accountEvmAddress: string = HAccountId.fromString(accountId).toSolidityAddress();
        tr = await th.revokeRole(stableCoinCapabilities2, accountEvmAddress, StableCoinRole.BURN_ROLE);
		tr = await th.hasRole(stableCoinCapabilities2, accountEvmAddress, StableCoinRole.BURN_ROLE);
	...
	it('Test get roles contract function', async () => {
        ...
        expect(tr.response).toEqual([StableCoinRole.DEFAULT_ADMIN_ROLE, StableCoinRole.CASHIN_ROLE, StableCoinRole.BURN_ROLE,
            StableCoinRole.WIPE_ROLE, StableCoinRole.RESCUE_ROLE, StableCoinRole.PAUSE_ROLE, StableCoinRole.FREEZE_ROLE,
	...
	it('Test increase supplier allowance contract function', async () => {
		const accountEvmAddress: string = HAccountId.fromString(accountId).toSolidityAddress();
		tr = await th.revokeSupplierRole(stableCoinCapabilities2, accountEvmAddress);
        ...
        tr = await th.isUnlimitedSupplierAllowance(stableCoinCapabilities2, accountEvmAddress);
        expect(tr.response).toEqual(false);        
    }, 20000);
