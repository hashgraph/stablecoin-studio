...
task('addNewVersionTokenManager', 'Add a new version TokenManager in factory')
    ...
    .setAction(async ({ tokenmanager, proxyfactory }: { tokenmanager: string; proxyfactory: string }, hre) => {
...
task('getTokenManager', 'Get TokenManager list in factory')
    ...
    .setAction(async ({ proxyfactory }: { proxyfactory: string }, hre) => {
        ...
        const tokenManagerAddress: string[] = await getHederaTokenManagerAddresses(
            ContractId.fromString(proxyfactory),
            client
        )
        console.log(
...
task('updateTokenManager', 'Update TokenManager in factory')
    ...
    .setAction(
        async (
            { tokenmanager, proxyfactory, index }: { tokenmanager: string; proxyfactory: string; index: number },
            hre
        ) => {
            const accounts = hre.network.config.accounts as unknown as Array<AccountHedera>
            const client = getClient(hre.network.name)
...
task('removeTokenManager', 'Remove TokenManager in factory')
    .addParam('proxyfactory', 'The proxy factory address')
    .addParam('index', 'Index you want to update')
    .setAction(async ({ proxyfactory, index }: { proxyfactory: string; index: number }, hre) => {
        const accounts = hre.network.config.accounts as unknown as Array<AccountHedera>
        const client = getClient(hre.network.name)
        ...
        await removeHederaTokenManagerVersion(ContractId.fromString(proxyfactory), client, index)
...
task('deployFactory', 'Deploy new factory').setAction(async (arguements: any, hre) => {
    ...
    client.setOperator(clientOneAccount, toHashgraphKey(clientOnePrivateKey, clientOneIsED25519))
    const tokenManager = await deployHederaTokenManager(client, clientOnePrivateKey)
    ...
    const result = await deployFactory(initializeFactory, client, clientOnePrivateKey, clientOneIsED25519)
...
task('getFactoryAdmin', 'Get the Factory admin account, the one with the right to add and remove tokenManagers')
    ...
    .setAction(
        async (
            ...
            const admin = await evmToHederaFormat(
                await getAdminStableCoinFactory(ContractId.fromString(proxyfactory), client)
            )
...
task('deployTokenManager', 'Deploy new TokenManager').setAction(async (arguements: any, hre) => {
    const accounts = hre.network.config.accounts as unknown as Array<AccountHedera>
    const client = getClient(hre.network.name)
