...
```solidity
function initialize(IHederaTokenService.HederaToken token, uint64 initialTotalSupply, uint32 tokenDecimals, address originalSender, address reserveAddress) external payable returns (address)
```
