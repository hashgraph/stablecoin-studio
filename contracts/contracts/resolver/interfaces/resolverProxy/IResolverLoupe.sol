...
interface IResolverLoupe is IDiamondLoupe, IStaticFunctionSelectors {
    ...
    /// @return resolverFacets_ Facet
    function getFacets() external view returns (ResolverFacet[] memory resolverFacets_);

    /// @notice Gets facet length.
    /// @return resolverFacetsLength_ Facets length
    function getFacetsLength() external view returns (uint256 resolverFacetsLength_);
    ...
    function getFacetsByPage(
        uint256 _pageIndex,
        uint256 _pageLength
    ) external view returns (ResolverFacet[] memory resolverFacets_);
    ...
    function getFacet(bytes32 _facetId) external view returns (ResolverFacet memory facet_);
