...
describe('Supplier Admin Tests - (roles)', function () {
    before(async function () {
        ...
        const result = await deployContractsWithSDK({
            ...
            initialAmountDataFeed: INIT_SUPPLY.add(BigNumber.from('150').mul(TOKEN_FACTOR)).toString(),
        })
    ...
    it('Admin account can grant and revoke supplier(s) role to an account', async function () {
        ...
        let Role = await hasRole(CASHIN_ROLE, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ...
        await grantSupplierRole(proxyAddress, cashInLimit, operatorClient, nonOperatorAccount, nonOperatorIsE25519)

        Role = await hasRole(CASHIN_ROLE, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        expect(Role).to.equals(true)
        result = await getSupplierAllowance(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        expect(result.toString()).to.eq(cashInLimit.toString())

        // Admin revokes limited supplier role : success
        await revokeSupplierRole(proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ...
        await grantUnlimitedSupplierRole(proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
    ...
    it('Admin account can increase, decrease and reset supplier(s) amount', async function () {
        ...
        await grantSupplierRole(proxyAddress, cashInLimit, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        await increaseSupplierAllowance(proxyAddress, amount, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        let result = await getSupplierAllowance(
            proxyAddress,
            nonOperatorClient,
            nonOperatorAccount,
        ...
        await decreaseSupplierAllowance(proxyAddress, amount, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        result = await getSupplierAllowance(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        expectedAmount = cashInLimit
        expect(result.toString()).to.eq(expectedAmount.toString())

        // Admin resets supplier allowance : success
        await resetSupplierAllowance(proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        result = await getSupplierAllowance(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        expect(result.toString()).to.eq('0')
    ...
    it('Non Admin account can not grant nor revoke supplier(s) role to an account', async function () {
        ...
        await expect(
            grantSupplierRole(proxyAddress, cashInLimit, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)

        // Non admin grants unlimited supplier role : fail
        await expect(
            grantUnlimitedSupplierRole(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)

        // Non admin revokes limited supplier role : fail
        await grantSupplierRole(proxyAddress, cashInLimit, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        await expect(
            revokeSupplierRole(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)
    ...
    it('Non Admin account can not increase, decrease and reset supplier(s) amount', async function () {
        ...
        await grantSupplierRole(proxyAddress, cashInLimit, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        await expect(
            increaseSupplierAllowance(proxyAddress, amount, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)

        // non Admin decreases supplier allowance : fail
        await expect(
            decreaseSupplierAllowance(proxyAddress, amount, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)

        // Non Admin resets supplier allowance : fail
        await expect(
            resetSupplierAllowance(proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)
...
describe('Supplier Admin Tests - (Unlimited)', function () {
    before(async function () {
        ...
        const result = await deployContractsWithSDK({
            ...
            publicKey: operatorPubKey,
            isED25519Type: operatorIsE25519,
            initialAmountDataFeed: INIT_SUPPLY.add(BigNumber.from('1500').mul(TOKEN_FACTOR)).toString(),
        })
    ...
    it('An account with unlimited supplier role can cash in 100 tokens', async function () {
        ...
        // Cashin tokens to previously associated account
        await Mint(proxyAddress, AmountToMint, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)

        // Check balance of account and total supply : success
        const finalTotalSupply = await getTotalSupply(proxyAddress, operatorClient)
        const finalBalanceOf = await getBalanceOf(proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        const expectedTotalSupply = initialTotalSupply.add(AmountToMint)
        const expectedBalanceOf = initialBalanceOf.add(AmountToMint)
    ...
    it('An account with unlimited supplier role can not be granted limited supplier role', async function () {
        ...
        await expect(
            grantSupplierRole(proxyAddress, BigNumber.from(1), operatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)
    })
...
describe('Supplier Admin Tests - (Limited)', function () {
    ...
    before(async function () {
        ...
        const result = await deployContractsWithSDK({
            ...
            publicKey: operatorPubKey,
            isED25519Type: operatorIsE25519,
            initialAmountDataFeed: INIT_SUPPLY.add(BigNumber.from('250').mul(TOKEN_FACTOR)).toString(),
        })
    ...
    it('An account with supplier role and an allowance of (100 + maxsupply) tokens can not cash more than maxSupply tokens', async function () {
        ...
        await expect(
            Mint(proxyAddress, MAX_SUPPLY.add(1), nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)
    })
    ...
    it('An account with supplier role and and allowance of 100 token can mint ALL tokens then allowance can be increased', async function () {
        ...
        // Cashin all tokens
        await Mint(proxyAddress, cashInLimit, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ...
        await Wipe(proxyAddress, extraAllowance, operatorClient, nonOperatorAccount, nonOperatorIsE25519)
    ...
    it('An account with supplier role and an allowance of 100 tokens, can mint 90 tokens but, later on, cannot mint 11 tokens', async function () {
        ...
        await expect(
            Mint(proxyAddress, amountToMintlater.add(1), nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)
        ).to.eventually.be.rejectedWith(Error)
    })
