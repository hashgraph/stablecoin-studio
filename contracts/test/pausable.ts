...
describe("Pause Tests", function() {
  ...
  before(async function  () {         
    ...
    it("Admin account can grant and revoke pause role to an account", async function() {    
      ...
      let result = await hasRole(PAUSE_ROLE, ContractId, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519);
      expect(result).to.equals(false);
  
      await grantRole(PAUSE_ROLE, ContractId, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519);
  
      result = await hasRole(PAUSE_ROLE, ContractId, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519);
    ...
    it("Non Admin account can not grant pause role to an account", async function() {   
      ...
      await expect(grantRole(PAUSE_ROLE, ContractId, proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)).to.eventually.be.rejectedWith(Error);
    });
  
    it("Non Admin account can not revoke pause role to an account", async function() {
      // Non Admin revokes pause role : fail       
      await grantRole(PAUSE_ROLE, ContractId, proxyAddress, operatorClient, nonOperatorAccount, nonOperatorIsE25519);
      await expect(revokeRole(PAUSE_ROLE, ContractId, proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)).to.eventually.be.rejectedWith(Error);
    ...
    it("A paused token can not be used for any other operation, like associating", async function() {
      ...
      await expect(associateToken(ContractId, proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)).to.eventually.be.rejectedWith(Error);
    ...
    it("An unpaused token can be used for any other operation, like associating", async function() {
      ...
      await pause(ContractId, proxyAddress, client2);
      await unpause(ContractId, proxyAddress, client2);
      await expect(associateToken(ContractId, proxyAddress, nonOperatorClient, nonOperatorAccount, nonOperatorIsE25519)).not.to.eventually.be.rejectedWith(Error);
