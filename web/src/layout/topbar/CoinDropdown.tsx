...
const CoinDropdown = () => {
        ...
        useEffect(() => {
                // Only redirect if we have a selected stablecoin AND capabilities
                if (capabilities?.capabilities.length !== 0 && isInStableCoinNotSelected && selectedStableCoin) {
                        RouterManager.to(navigate, NamedRoutes.Operations);
                }
        }, [capabilities, selectedStableCoin]);
        ...
        const handleSelectCoin = async (event: any) => {
                ...
                try {
                        ...
                        const stableCoinConfigInfo: any = await Promise.race([
                                SDKService.getConfigInfo(
                                ...
                                new Promise((resolve, reject) => {
                                        setTimeout(() => {
                                                reject(new Error("Stablecoin Config info couldn't be obtained in a reasonable time."));
                                        }, 10000);
                                }),
                        ]).catch((e) => {
                                console.log(e.message);
