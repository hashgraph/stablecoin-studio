...
export const eFormatAmount = ({ amount, decimals }: { amount?: number; decimals?: number }) => {
	if (typeof amount !== 'number' || Number.isNaN(amount)) return '';
...
export const formatAmountWithDecimals = ({ amount, decimals, language = 'en-EN',}: { amount: number; decimals: number; language?: string; }) => {
