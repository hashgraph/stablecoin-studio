...
describe(`<${walletSlice.name} />`, () => {
	describe('reducers', () => {
		...
		it('Should be able to setLastWallet', async () => {
			store.dispatch(
				walletSlice.actions.setLastWallet(
		...
		it('Should be able to setData', async () => {
			...
			store.dispatch(
				walletSlice.actions.setData({ account })
			)
		...
		it('Should be able to setAccount', async () => {
			const account = 'account';
			store.dispatch(
				walletSlice.actions.setAccount(account)
			)
		...
		it('Should be able to setSelectedStableCoin', async () => {
			...
			store.dispatch(
				walletSlice.actions.setSelectedStableCoin({ tokenId })
			)
		...
		it('Should be able to setSelectedStableCoinProxyConfig', async () => {
			...
			expect(store.getState().selectedStableCoinProxyConfig?.implementationAddress).toEqual(implementationAddress)
		...
		it('Should be able to setSelectedNetworkFactoryProxyConfig', async () => {
			...
			expect(store.getState().selectedNetworkFactoryProxyConfig?.implementationAddress).toEqual(implementationAddress)
		...
		it('Should be able to setSelectingStableCoin', async () => {
			store.dispatch(
				walletSlice.actions.setSelectingStableCoin(true)
			)
		...
		it('Should be able to setStableCoinList', async () => {
			...
			store.dispatch(
				walletSlice.actions.setStableCoinList({ coins })
			)
		...
		it('Should be able to setExternalTokenList', async () => {
			...
			store.dispatch(
				walletSlice.actions.setExternalTokenList([externalTokenList])
			)
		...
		it('Should be able to setHasWalletExtension', async () => {
			store.dispatch(
				walletSlice.actions.setHasWalletExtension(SupportedWallets.HASHPACK)
			)
		...
		it('Should be able to setCapabilities', async () => {
			...
			store.dispatch(
				walletSlice.actions.setCapabilities(stableCoinCapabilities)
			)
		...
		it('Should be able to setAccountInfo', async () => {
			const id = 'id';
			store.dispatch(
				walletSlice.actions.setAccountInfo({id}),
			)
		...
		it('Should be able to setStatus', async () => {
			...
			store.dispatch(
				walletSlice.actions.setStatus(status)
			)
		...
		it('Should be able to setPausedToken', async () => {
			...
			store.dispatch(
				walletSlice.actions.setPausedToken(pausedToken)
			)
		...
		it('Should be able to setDeletedToken', async () => {
			...
			store.dispatch(
				walletSlice.actions.setDeletedToken(deletedToken)
			)
		...
		it('Should be able to setNetwork', async () => {
			...
			store.dispatch(
				walletSlice.actions.setNetwork(network)
			)
		...
		it('Should be able to setNetworkRecognized', async () => {
			...
			store.dispatch(
				walletSlice.actions.setNetworkRecognized(networkRecognized)
			)
		...
		it('Should be able to setIsProxyOwner', async () => {
			...
			store.dispatch(
				walletSlice.actions.setIsProxyOwner(isProxyOwner)
			)
		...
		it('Should be able to setIsFactoryProxyOwner', async () => {
			...
			store.dispatch(
				walletSlice.actions.setIsFactoryProxyOwner(isFactoryProxyOwner)
			)
		...
		it('Should be able to setAccountRecognized', async () => {
			...
			store.dispatch(
				walletSlice.actions.setAccountRecognized(accountRecognized)
			)
		...
		it('Should be able to setFactoryId', async () => {
			...
			store.dispatch(
				walletSlice.actions.setFactoryId(factoryId)
			)
		...
		it('Should be able to setRoles', async () => {
			store.dispatch(
				walletSlice.actions.setRoles(['role'])
			)
		...
		it('Should be able to clearSelectedStableCoinProxyConfig', async () => {
			...
			expect(store.getState().selectedStableCoinProxyConfig).toEqual(initialState.selectedStableCoinProxyConfig)
			expect(store.getState().isProxyOwner).toEqual(initialState.isProxyOwner)
		});
		...
		it('Should be able to clearSelectedNetworkFactoryProxyConfig', async () => {
			...
			expect(store.getState().selectedNetworkFactoryProxyConfig).toEqual(initialState.selectedNetworkFactoryProxyConfig)
			expect(store.getState().isProxyOwner).toEqual(initialState.isProxyOwner)
		});		
