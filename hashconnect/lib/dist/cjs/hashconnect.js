...
const mirror_node_client_js_1 = require("./mirror-node/mirror-node-client.js");
...
class HashConnect {
    ...
    async pair(pairingData, accounts, network) {
        ...
        if (pairingData.metadata.publicKey)
            this.encryptionKeys[pairingData.topic] = pairingData.metadata
                .publicKey; //todo: remove as backwards compatibility
    ...
    async getSignerWithAccountKey(provider) {
        const client = new mirror_node_client_js_1.MirrorNodeClient(provider.network);
        const accountInfo = await client.getAccountInfo(provider.accountToSign);
        const publicKey = sdk_1.PublicKey.fromString(accountInfo.key.key);
        return new signer_1.HashConnectSigner(this, provider, provider.accountToSign, provider.topicId, publicKey);
    }
