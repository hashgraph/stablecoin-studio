import { AccountId, TransactionId, AccountBalanceQuery, AccountInfoQuery, AccountRecordsQuery, TransactionReceipt, } from "@hashgraph/sdk";
export class HashConnectSigner {
    constructor(hashconnect, provider, accountToSign, topic, key) {
        ...
        try {
            ...
            if (out.response && typeof res.response === "object" && res.receipt) {
                out.getReceiptWithSigner = (signer) => {
                    signer;
                    return new Promise((resolve) => resolve(TransactionReceipt.fromBytes(res.receipt)));
                };
            }
        }
        catch (err) {
