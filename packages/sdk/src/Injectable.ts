...
import { Constructor } from './core/Type.js';
...
import { CommandHandlerType } from './core/command/CommandBus.js';
import { QueryHandlerType } from './core/query/QueryBus.js';
import NetworkService, { NetworkProps } from './app/service/NetworkService.js';
...
import { GrantUnlimitedSupplierRoleCommandHandler } from './app/usecase/command/stablecoin/roles/grantUnlimitedSupplierRole/GrantUnlimitedSupplierRoleCommandHandler.js';
...
import { ExecuteHoldCommandHandler } from './app/usecase/command/stablecoin/operations/hold/executeHold/ExecuteHoldCommandHandler.js';
...
import { GetCapabilitiesQueryHandler } from './app/usecase/query/stablecoin/capabilities/GetCapabilitiesQueryHandler.js';
...
import { NetworkConfiguratorImpl } from './app/service/NetworkConfiguratorImpl.js';
import { TransactionDescriber } from './core/service/TransactionDescriber.js';
import { TransactionDescriberImpl } from './app/service/TransactionDescriberImpl.js';
export { TOKENS };
...
const QUERY_HANDLERS = [
	...
	{
		token: TOKENS.QUERY_HANDLER,
		useClass: GetAccountAutoAssociationQueryHandler,
	},
	{
		token: TOKENS.QUERY_HANDLER,
	...
	{
		token: TOKENS.QUERY_HANDLER,
		useClass: GetCapabilitiesQueryHandler,
	},
];
...
// Abstract → Concrete adapter bindings (DIP)
container.register(AbstractMirrorNodeAdapter, {
	useFactory: (c: DependencyContainer) => c.resolve(MirrorNodeAdapter),
});
container.register(AbstractRPCQueryAdapter, {
	useFactory: (c: DependencyContainer) => c.resolve(RPCQueryAdapter),
});
container.register(AbstractBackendAdapter, {
	useFactory: (c: DependencyContainer) => c.resolve(BackendAdapter),
});
container.register(AbstractNetworkService, {
	useFactory: (c: DependencyContainer) => c.resolve(NetworkService),
});
container.register(AbstractEventService, {
	useFactory: (c: DependencyContainer) => c.resolve(EventService),
});
container.register(NetworkConfigurator, {
	useFactory: (c: DependencyContainer) => c.resolve(NetworkConfiguratorImpl),
});
container.register(TransactionDescriber, {
	useFactory: (c: DependencyContainer) => c.resolve(TransactionDescriberImpl),
});
