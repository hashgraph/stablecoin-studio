...
export class BuildHederaStep implements ExecutionStep {
  ...
  async execute(ctx: ExecutionContext): Promise<ExecutionContext> {
    ...
    const contract = (ctx.params.contractAddress as string) ?? '';
    const gas = (transaction as any)._maxGasAllowanceHbar ?? (transaction as any)._gas;
    // Log business params (exclude internal/builder fields)
    const paramEntries = Object.entries(ctx.params)
      .filter(([k]) => k !== 'contractAddress' && k !== 'abi')
      .map(([k, v]) => `${k}=${v}`)
      .join(', ');
    logger.step(`     contract=${contract}${gas ? ` gas=${gas}` : ''}`);
