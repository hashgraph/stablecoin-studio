...
import { ParseHederaReceiptStep } from '../../../../core/dlt/hedera/steps/ParseHederaReceiptStep.js';
import { ParseEVMReceiptStep } from '../../../../core/dlt/evm/steps/ParseEVMReceiptStep.js';
...
describe('ParseHederaReceiptStep', () => {
  ...
  it('calls getReceipt on the response with the client', async () => {
    ...
    const mockResponse = { getRecord: jest.fn().mockResolvedValue(mockReceipt) };
  ...
  it('propagates the rest of the context unchanged', async () => {
    const mockResponse = { getRecord: jest.fn().mockResolvedValue({}) };
    const mockClient = {} as any;
