...
import { BuildHederaStep } from '../../../../core/dlt/hedera/steps/BuildHederaStep.js';
import { BuildEVMStep } from '../../../../core/dlt/evm/steps/BuildEVMStep.js';
...
describe('BuildHederaStep', () => {
  ...
  it('calls builder.buildHederaTransaction with ctx.params', async () => {
    const step = new BuildHederaStep();
    const ctx = makeMockContext();

    await step.execute(ctx);

    expect(ctx.builder.buildHederaTransaction).toHaveBeenCalledWith({ foo: 'bar' });
  });

  it('returns context with transaction set from builder result', async () => {
    const step = new BuildHederaStep();
    const ctx = makeMockContext();
  ...
  it('propagates the rest of the context without modification', async () => {
    ...
    expect(result.builder).toBe(ctx.builder);
  });
...
describe('BuildEVMStep', () => {
  ...
  it('calls builder.buildEVMTransaction with ctx.params', async () => {
    const step = new BuildEVMStep();
    const ctx = makeMockContext();

    await step.execute(ctx);

    expect(ctx.builder.buildEVMTransaction).toHaveBeenCalledWith({ foo: 'bar' });
  });
  ...
  it('awaits the async buildEVMTransaction result', async () => {
    ...
    const ctx = makeMockContext();
    (ctx.builder.buildEVMTransaction as jest.Mock).mockResolvedValue(resolvedTx);
