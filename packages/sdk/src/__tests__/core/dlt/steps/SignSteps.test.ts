...
import { SignWithClientStep } from '../../../../core/dlt/hedera/steps/SignWithClientStep.js';
import { SignWithSignerStep } from '../../../../core/dlt/evm/steps/SignWithSignerStep.js';
import {
  SignWithExternalStep,
  ExternalSigningClient,
} from '../../../../core/dlt/shared/SignWithExternalStep.js';
...
describe('SignWithClientStep', () => {
  ...
  it('freezes transaction with client and signs with provided private key', async () => {
    ...
    const step = new SignWithClientStep(mockClient, mockPrivateKey);
  ...
  it('sets signedTransaction with kind hedera', async () => {
    ...
    const step = new SignWithClientStep(mockClient, {} as any);
    const ctx = makeMockContext({ transaction: mockTx as any });
...
describe('SignWithSignerStep', () => {
  ...
  it('calls signer.signTransaction with the transaction', async () => {
    ...
    const mockSigner = {
      populateTransaction: jest.fn().mockResolvedValue(mockTx),
    ...
    expect(mockSigner.populateTransaction).toHaveBeenCalledWith(mockTx);
  ...
  it('propagates the rest of the context', async () => {
    const mockSigner = {
      populateTransaction: jest.fn().mockImplementation((tx: any) => Promise.resolve(tx)),
