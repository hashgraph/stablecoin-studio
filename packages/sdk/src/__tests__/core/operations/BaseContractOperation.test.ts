...
import { BaseContractOperation } from '../../../core/operations/BaseContractOperation.js';
import { OperationParams, OperationResult } from '../../../core/operations/types.js';

// Handler concreto de prueba
interface TestParams extends OperationParams {
  amount: string;
}

interface TestResult extends OperationResult {
  processedAmount: string;
}

class TestHandler extends BaseContractOperation<TestParams, TestResult> {
  constructor() {
...
class NoParamsHandler extends BaseContractOperation<OperationParams, OperationResult> {
  constructor() {
  ...
  protected createResult(receipt: any): OperationResult {
    return { success: true, transactionId: receipt?.transactionId ?? '' };
  }
}

// Handler solo Hedera
class HederaOnlyHandler extends BaseContractOperation<TestParams, TestResult> {
  constructor() {
...
describe('BaseContractOperation', () => {
  ...
  describe('extractResult', () => {
    it('should create result from receipt', () => {
      ...
      const result = handler.extractResult(receipt, { amount: '500' });
